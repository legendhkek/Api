...
if (!defined('PROXY_HEALTH_MAX_CONCURRENCY')) {
    define('PROXY_HEALTH_MAX_CONCURRENCY', 200);
}
if (!defined('PROXY_HEALTH_DEFAULT_CONCURRENCY')) {
    define('PROXY_HEALTH_DEFAULT_CONCURRENCY', 60);
}

$healthConcurrencyRaw = PROXY_HEALTH_DEFAULT_CONCURRENCY;
if (php_sapi_name() === 'cli' && isset($argv)) {
    for ($i = 0, $count = count($argv); $i < $count; $i++) {
        $arg = $argv[$i];
        if (strpos($arg, '--concurrency=') === 0) {
            $healthConcurrencyRaw = (int)substr($arg, strlen('--concurrency='));
        } elseif ($arg === '--concurrency' && isset($argv[$i + 1])) {
            $healthConcurrencyRaw = (int)$argv[$i + 1];
            $i++;
        }
    }
} elseif (isset($_GET['concurrency'])) {
    $healthConcurrencyRaw = (int)$_GET['concurrency'];
}
$healthConcurrency = $healthConcurrencyRaw > 0
    ? max(1, min($healthConcurrencyRaw, PROXY_HEALTH_MAX_CONCURRENCY))
    : PROXY_HEALTH_DEFAULT_CONCURRENCY;
 ...
 * Keeps timeouts unchanged. Default concurrency: 60 (max 200)
 * Returns arrays of working and dead proxies
 */
function testProxiesInParallelHealth(array $proxies, int $timeout = 5, int $concurrency = PROXY_HEALTH_DEFAULT_CONCURRENCY) {
    $workingProxies = [];
    $deadProxies = [];
    $total = count($proxies);
    ...
    $chunkSize = max(1, min($concurrency, PROXY_HEALTH_MAX_CONCURRENCY));
    $chunks = array_chunk($proxies, $chunkSize);
    $tested = 0;
    $batch = 0;
    $totalBatches = max(1, count($chunks));

    foreach ($chunks as $chunk) {
        $batch++;
        $mh = curl_multi_init();
        $handles = [];
        echo "⚙ Batch {$batch}/{$totalBatches} | concurrency {$chunkSize}\n";
 ...
 */
function checkAndFilterProxies($proxyListFile, int $concurrency = PROXY_HEALTH_DEFAULT_CONCURRENCY) {
    ...
    // Parallel health check with configurable concurrency (5s timeout each)
    list($workingProxies, $deadProxies) = testProxiesInParallelHealth($cleanProxies, 5, $concurrency);
 ...
 */
function updateProxyList(int $concurrency = PROXY_HEALTH_DEFAULT_CONCURRENCY) {
    global $cacheFile, $proxyListFile;
    
    echo "🔄 Checking proxy health...\n\n";
    echo "⚙ Concurrency: {$concurrency} (max " . PROXY_HEALTH_MAX_CONCURRENCY . ")\n";
    
    $result = checkAndFilterProxies($proxyListFile, $concurrency);
...
if (php_sapi_name() === 'cli') {
    ...
    if (shouldCheckProxies($cacheFile, $checkInterval)) {
        ...
        updateProxyList($healthConcurrency);
    } else {
    ...
    if ($needsCheck && isset($_GET['auto'])) {
        ob_start();
        $success = updateProxyList($healthConcurrency);
        $output = ob_get_clean();
        $lastCheck = time();
    ...
    echo json_encode([
        ...
        'default_concurrency' => PROXY_HEALTH_DEFAULT_CONCURRENCY,
        'max_concurrency' => PROXY_HEALTH_MAX_CONCURRENCY,
