 ...
 * Keeps timeouts unchanged. Default concurrency: 20
 * Returns arrays of working and dead proxies
 */
function testProxiesInParallelHealth(array $proxies, int $timeout = 5, int $concurrency = 200) {
    $workingProxies = [];
    $deadProxies = [];
    $total = count($proxies);
...
function checkAndFilterProxies($proxyListFile) {
    ...
    // Parallel health check (20 at a time, 5s timeout each)
    list($workingProxies, $deadProxies) = testProxiesInParallelHealth($cleanProxies, 5, 20);
