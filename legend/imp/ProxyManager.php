...
class ProxyManager {
    ...
    private $maxConcurrency = 200; // Support up to 200 concurrent connections
    ...
    /**
     * Check multiple proxies in parallel (up to maxConcurrency)
     * 
     * @param array $proxies Array of proxy data arrays
     * @param string $testUrl Optional custom test URL
     * @return array Array of ['proxy' => proxy_data, 'healthy' => bool] results
     */
    public function checkProxiesHealthParallel(array $proxies, string $testUrl = ''): array {
        $url = $testUrl ?: $this->healthCheckUrl;
        $results = [];
        
        // Process in chunks to respect maxConcurrency
        $chunks = array_chunk($proxies, $this->maxConcurrency);
     ...
     * 
     * @param int $maxConcurrency Maximum concurrent connections (default: 200)
     */
    public function setMaxConcurrency(int $maxConcurrency): void {
        $this->maxConcurrency = max(1, min(200, $maxConcurrency)); // Limit between 1-200
    }
    
    /**
     * Get maximum concurrency setting
     * 
     * @return int Current max concurrency value
     */
