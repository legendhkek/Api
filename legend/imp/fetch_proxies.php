 ...
 *     - concurrency (int): parallel tests per batch (default: 40, max: 200)
...
if (!defined('PROXY_FETCHER_MAX_CONCURRENCY')) {
    define('PROXY_FETCHER_MAX_CONCURRENCY', 200);
}
if (!defined('PROXY_FETCHER_DEFAULT_CONCURRENCY')) {
    define('PROXY_FETCHER_DEFAULT_CONCURRENCY', 40);
}
...
$rawConcurrency = null;
if (isset($_GET['concurrency_override']) && $_GET['concurrency_override'] !== '') {
    $rawConcurrency = (int)$_GET['concurrency_override'];
} elseif (isset($_GET['concurrency'])) {
    $rawConcurrency = (int)$_GET['concurrency'];
}
if ($rawConcurrency === null || $rawConcurrency <= 0) {
    $concurrency = PROXY_FETCHER_DEFAULT_CONCURRENCY;
} else {
    $concurrency = max(1, min($rawConcurrency, PROXY_FETCHER_MAX_CONCURRENCY));
}
 ...
 * Keeps timeouts unchanged, defaults: timeout=3s, concurrency=40
 * If $targetWorking <= 0, tests ALL proxies (no early stop)
 */
function testProxiesInParallel(array $proxies, int $timeout = 3, int $concurrency = PROXY_FETCHER_DEFAULT_CONCURRENCY, int $targetWorking = 0): array {
    $total = count($proxies);
    $tested = 0;
    $working = [];
    $workingDetails = [];
    ...
    $chunkSize = max(1, min($concurrency, PROXY_FETCHER_MAX_CONCURRENCY));
    ...
    foreach ($chunks as $chunk) {
        ...
        $mh = curl_multi_init();
        $handles = [];
        logMessage("🚀 Batch {$chunkIndex}/{$totalBatches} | concurrency {$chunkSize}", 'info');
...
logMessage("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━", 'info');
logMessage("⚙ Concurrency configured: {$concurrency} (max " . PROXY_FETCHER_MAX_CONCURRENCY . ")", 'info');
        ...
        echo json_encode([
            ...
            'stats' => [
                ...
                'parameters' => [
                    ...
                    'max_concurrency' => PROXY_FETCHER_MAX_CONCURRENCY,
