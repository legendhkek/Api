 ...
 *     - concurrency (int): parallel tests per batch (default: 20)
...
$concurrency = isset($_GET['concurrency']) ? max(1, min(200, (int)$_GET['concurrency'])) : 200;
 ...
 * Keeps timeouts unchanged, defaults: timeout=3s, concurrency=20
 * If $targetWorking <= 0, tests ALL proxies (no early stop)
 */
function testProxiesInParallel(array $proxies, int $timeout = 3, int $concurrency = 200, int $targetWorking = 0): array {
    $total = count($proxies);
    $tested = 0;
    $working = [];
    $workingDetails = [];
